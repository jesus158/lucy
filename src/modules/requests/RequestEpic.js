  ...
  action$.pipe(
    ...
    mergeMap(action =>
      Observable.create(obs => {
        ...
        axios(RequestApiClient.createRequest(action.onSucess, action.idUbicationBegin, action.idUbicationEnd, action.idTypeService, action.idAsset, action.freeAsset, action.freeAssetTime, action.idCarrierUserSystemAccount, action.moreInformation, action.nameRequested))
          .then(response => {
            let code = response.data.apiResponse.code;
...
export const updateRequest = (action$, state$) =>
    action$.pipe(
        ...
        mergeMap(action =>
            Observable.create(obs => {
                axios.defaults.timeout = apiTimeout;
                axios(RequestApiClient.updateRequest(action.idUpdateRequest, action.idUbicationBegin, action.idUbicationEnd, action.idTypeService, action.idAsset, action.freeAsset, action.freeAssetTime, action.idCarrierUserSystemAccount, action.newInfo, action.nameRequested))
                    .then(response => {
                        let code = response.data.apiResponse.code;
                        if (response.status >= 200 && response.status < 300 && code === 200) {
                            let data = response.data;
