...
class ApiClient {
  ...
  getHistoryRequest = (
    ...
    if (idTypeService !== undefined) {
        uriFilter += "&idTypeService=" + idTypeService;
    }
    
    if (stateRequest !== undefined) {
        uriFilter += "&state=" + stateRequest;
    }

    if (idCarrier !== undefined) {
        uriFilter += "&IdCarrier=" + idCarrier;
    }

    if (locationBegin !== undefined) {
        uriFilter += "&idLocationBegin=" + locationBegin;
    }

    if (locationEnd !== undefined) {
        uriFilter += "&idLocationEnd=" + locationEnd;
    }
    ...
    let request = {
      method: "get",
      url:
        `${this.url}/reports/getRequests/${this.account}/` + encodeURI(uriFilter),
      headers: {
