...
import TypeAssetApi from 'modules/configurations/type_actives/TypeActivesApiClient.js';
import { configureTypeAssetError, configureTypeAssetSuccess, CONFIGURE_TYPE_ASSET, findTypeAssetError, findTypeAssetSuccess, FIND_TYPE_ASSET_LIST, getListActiveTypeAssetError, getListActiveTypeAssetSuccess, getTypeAssetByIdError, getTypeAssetByIdSuccess, GET_LIST_ACTIVE_TYPE_ASSET, GET_TYPE_ASSET_BY_ID, inactiveTypeAssetError, inactiveTypeAssetSuccess, INACTIVE_TYPE_ASSET } from 'modules/configurations/type_actives/TypeActivesActions.js';
...
export const getListActiveTypeAsset = (action$, state$) => action$.pipe(
    ofType(GET_LIST_ACTIVE_TYPE_ASSET), mergeMap(action =>
        Observable.create(obs => {
            axios.defaults.timeout = apiTimeout;
            axios(TypeAssetApi.getListActiveTypeAsset())
                .then(response => {
                    let code = response.data.apiResponse.code;
                    if (response.status >= 200 && response.status < 300 && code === 200) {
                        let data = response.data;
                        obs.next(getListActiveTypeAssetSuccess(data.listTypeAsset));
                        //obs.next(addMessage({variant:"success", message:response.data.apiResponse.message}));
                        obs.complete();
                        action.onSuccess("OK"); //Callback de suceso
                    } else if (response.status === 401) {
                        obs.next(getListActiveTypeAssetError(response.data.apiResponse.message));
                ...
                .catch(error => {
                    obs.next(getListActiveTypeAssetError(error));
                    //obs.next(addMessage({variant:"error", message:error.message}));
                    obs.complete();
                    action.onSuccess("ERROR");
                });
        }).pipe(catchError(error => of (getListActiveTypeAssetError(error)
